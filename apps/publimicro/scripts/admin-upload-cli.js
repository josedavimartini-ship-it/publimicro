...
async function main() {
  ...
  if (argv.length < 2) {
    console.error('Usage: admin-upload-cli <zip-path> <property-id> --admin-key <key> [--url <api-url>]');
    process.exit(2);
  }
  ...
  if (!adminKey) {
    console.error('Admin key is required via --admin-key or ADMIN_API_KEY env');
    process.exit(2);
  }
