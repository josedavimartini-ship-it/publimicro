          ...
          <div className="relative rounded-3xl overflow-hidden shadow-2xl border-4 border-[#0D7377]/40 min-h-[320px] group bg-gradient-to-r from-[#0D7377]/10 to-[#B7791F]/10 mb-16">
            ...
            <div className="relative p-10 z-20 flex flex-col md:flex-row items-center gap-10">
              <div className="flex-1">
                ...
                {/* Supabase Ranches Gallery */}
                <div className="mt-8">
                  <h3 className="text-2xl font-bold text-[#B7791F] mb-4">Sítios Disponíveis</h3>
                  {loading ? (
                    <p className="text-[#FF6B35]">Carregando ranchos...</p>
                  ) : errorMsg ? (
                    <p className="text-red-500">{errorMsg}</p>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {sitios.map((sitio) => (
                        <div key={sitio.id} className="bg-[#0D7377]/10 border border-[#0D7377]/30 rounded-xl shadow-lg p-4 flex flex-col gap-2">
                          <h4 className="text-lg font-bold text-[#FF6B35]">{sitio.nome}</h4>
                          <p className="text-[#d8c68e]">{sitio.localizacao}</p>
                          <p className="text-[#B7791F] font-bold">R$ {sitio.preco?.toLocaleString('pt-BR')}</p>
                          <div className="flex gap-2 overflow-x-auto">
                            {sitio.fotos && sitio.fotos.length > 0 ? (
                              sitio.fotos.map((foto, idx) => (
                                <Image
                                  key={idx}
                                  src={foto}
                                  alt={`Foto do rancho ${sitio.nome}`}
                                  width={120}
                                  height={80}
                              ...
                              <span className="text-[#FF6B35]">Sem fotos</span>
                            )}
                          </div>
                        </div>
                      ))}
            ...
            <div className="col-span-12 md:col-span-6 text-center py-8">
              ...
              <div className="flex flex-wrap gap-4 justify-center mt-8">
                ...
                <button
                  ...
                  onClick={() => {
                    if (window && window.openChatbot) {
                      window.openChatbot();
                    } else {
                      alert('Chatbot indisponível. Tente novamente mais tarde ou fale com um agente.');
                    }
                  }}
                >
                  <Sparkles className="w-5 h-5 text-[#FF6B35]" />
