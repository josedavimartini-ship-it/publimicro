...
DO $$ BEGIN RAISE NOTICE 'placeholder 20251107000005'; END $$;
-- ============================================
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policy WHERE polname = 'sitios_select_public' AND polrelid = 'public.sitios'::regclass) THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policy WHERE polname = 'sitios_insert_authenticated' AND polrelid = 'public.sitios'::regclass) THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policy WHERE polname = 'sitios_update_owner' AND polrelid = 'public.sitios'::regclass) THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policy WHERE polname = 'sitios_delete_owner' AND polrelid = 'public.sitios'::regclass) THEN
