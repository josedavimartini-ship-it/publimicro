...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relkind = 'i' AND relname = 'idx_user_verifications_user') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relkind = 'i' AND relname = 'idx_user_verifications_status') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relkind = 'i' AND relname = 'idx_user_verifications_cpf') THEN
    CREATE INDEX IF NOT EXISTS idx_user_verifications_cpf ON public.user_verifications(cpf) WHERE cpf IS NOT NULL;
  END IF;
EXCEPTION WHEN duplicate_table OR duplicate_object THEN NULL;
END$$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relkind = 'i' AND relname = 'idx_user_verifications_pending_review') THEN
    CREATE INDEX IF NOT EXISTS idx_user_verifications_pending_review ON public.user_verifications(status, created_at) 
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relkind = 'i' AND relname = 'idx_user_verifications_risk_level') THEN
    CREATE INDEX IF NOT EXISTS idx_user_verifications_risk_level ON public.user_verifications(risk_level, risk_score DESC);
  END IF;
EXCEPTION WHEN duplicate_table OR duplicate_object THEN NULL;
END$$;
...
CREATE INDEX IF NOT EXISTS idx_verification_audit_verification ON public.verification_audit_log(user_verification_id);
CREATE INDEX IF NOT EXISTS idx_verification_audit_user ON public.verification_audit_log(user_id);
CREATE INDEX IF NOT EXISTS idx_verification_audit_event ON public.verification_audit_log(event_type, created_at DESC);
...
-- Users can view their own verification (idempotent)
...
DROP POLICY IF EXISTS "Users can view own audit log" ON public.verification_audit_log;
CREATE POLICY "Users can view own audit log" ON public.verification_audit_log
  FOR SELECT TO authenticated
  USING (auth.uid() = user_id);

-- Anyone can view active verification rules
DROP POLICY IF EXISTS "Anyone can view active rules" ON public.verification_rules;
CREATE POLICY "Anyone can view active rules" ON public.verification_rules
  FOR SELECT TO authenticated
  USING (active = TRUE);
...
DROP TRIGGER IF EXISTS calculate_user_risk_score ON public.user_verifications;
...
DROP TRIGGER IF EXISTS apply_auto_verification_rules ON public.user_verifications;
...
DROP TRIGGER IF EXISTS log_verification_status_changes ON public.user_verifications;
...
CREATE TABLE IF NOT EXISTS public.user_verifications (
  ...
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (status IN (
    'pending','checking','approved','manual_review','rejected','suspended','appealing'
  )),
  full_name VARCHAR(200),
  cpf VARCHAR(14),
  date_of_birth DATE,
  ...
  criminal_record_status VARCHAR(20) DEFAULT 'not_checked',
  criminal_record_details JSONB DEFAULT '{}'::jsonb,
  criminal_check_date TIMESTAMP WITH TIME ZONE,
  credit_score INT,
  credit_status VARCHAR(20) DEFAULT 'not_checked',
  credit_restrictions JSONB DEFAULT '[]'::jsonb,
  credit_check_date TIMESTAMP WITH TIME ZONE,
  ...
  external_check_provider VARCHAR(50),
  external_check_response JSONB,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  approved_at TIMESTAMP WITH TIME ZONE,
...
CREATE INDEX IF NOT EXISTS idx_user_verifications_status ON public.user_verifications(status);
CREATE INDEX IF NOT EXISTS idx_user_verifications_cpf ON public.user_verifications(cpf) WHERE cpf IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_user_verifications_pending_review ON public.user_verifications(status, created_at) WHERE status IN ('pending', 'manual_review');
CREATE INDEX IF NOT EXISTS idx_user_verifications_risk_level ON public.user_verifications(risk_level, risk_score DESC);
...
DECLARE
  ...
  risk_factors_array JSONB := '[]'::jsonb;
BEGIN
  IF NEW.criminal_record_status = 'serious_crimes' THEN
    score := score + 50;
    risk_factors_array := risk_factors_array || '["serious_criminal_history"]'::jsonb;
  ELSIF NEW.criminal_record_status = 'criminal_history' THEN
    score := score + 30;
    risk_factors_array := risk_factors_array || '["criminal_history"]'::jsonb;
  ELSIF NEW.criminal_record_status = 'pending_cases' THEN
    score := score + 20;
    risk_factors_array := risk_factors_array || '["pending_legal_cases"]'::jsonb;
  END IF;
  IF NEW.credit_status = 'restricted' THEN score := score + 25; risk_factors_array := risk_factors_array || '["credit_restricted"]'::jsonb; END IF;
  IF NEW.credit_status = 'poor' THEN score := score + 15; risk_factors_array := risk_factors_array || '["poor_credit_score"]'::jsonb; END IF;
  IF NEW.cpf_valid = FALSE THEN score := score + 30; risk_factors_array := risk_factors_array || '["invalid_cpf"]'::jsonb; END IF;
  IF NEW.face_match_score IS NOT NULL AND NEW.face_match_score < 70 THEN score := score + 20; risk_factors_array := risk_factors_array || '["low_face_match_score"]'::jsonb; END IF;
  IF NEW.liveness_check_passed = FALSE THEN score := score + 15; risk_factors_array := risk_factors_array || '["failed_liveness_check"]'::jsonb; END IF;
  IF NEW.phone_verified = FALSE THEN score := score + 10; risk_factors_array := risk_factors_array || '["phone_not_verified"]'::jsonb; END IF;
  IF NEW.email_verified = FALSE THEN score := score + 10; risk_factors_array := risk_factors_array || '["email_not_verified"]'::jsonb; END IF;
  IF score > 100 THEN score := 100; END IF;
  NEW.risk_score := score;
  NEW.risk_factors := risk_factors_array;
  IF score >= 70 THEN NEW.risk_level := 'critical'; ELSIF score >= 50 THEN NEW.risk_level := 'high'; ELSIF score >= 30 THEN NEW.risk_level := 'medium'; ELSE NEW.risk_level := 'low'; END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
DECLARE
  ...
  needs_review BOOLEAN := FALSE;
BEGIN
  IF NEW.status != 'checking' THEN RETURN NEW; END IF;
  FOR rule IN SELECT * FROM verification_rules WHERE active = TRUE ORDER BY priority DESC LOOP
    IF rule.action = 'auto_reject' AND NEW.criminal_record_status = 'serious_crimes' THEN
      NEW.status := 'rejected'; NEW.rejection_reason := 'criminal_history'; NEW.rejection_details := 'Histórico criminal grave detectado nos sistemas oficiais.'; NEW.rejected_at := NOW(); should_reject := TRUE; EXIT;
    END IF;
    IF rule.action = 'manual_review' THEN
      IF NEW.criminal_record_status IN ('pending_cases', 'criminal_history') THEN
        needs_review := TRUE; NEW.requires_manual_review := TRUE; NEW.manual_review_reason := 'Processos judiciais ou histórico criminal requer análise manual.';
      END IF;
      IF NEW.risk_score >= 70 THEN needs_review := TRUE; NEW.requires_manual_review := TRUE; NEW.manual_review_reason := 'Pontuação de risco elevada.'; END IF;
    END IF;
    IF rule.action = 'auto_approve' AND NEW.criminal_record_status = 'clean' AND NEW.cpf_valid = TRUE AND NEW.risk_score < 30 THEN should_approve := TRUE; ELSE should_approve := FALSE; END IF;
  END LOOP;
  IF should_reject THEN NEW.status := 'rejected'; ELSIF needs_review THEN NEW.status := 'manual_review'; ELSIF should_approve AND NOT needs_review THEN NEW.status := 'approved'; NEW.approved_at := NOW(); ELSE NEW.status := 'manual_review'; END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  IF TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status THEN
    INSERT INTO verification_audit_log ( user_verification_id, user_id, event_type, old_status, new_status, performed_by_type, event_data ) VALUES ( NEW.id, NEW.user_id, 'status_changed', OLD.status, NEW.status, 'system', json_build_object('risk_score', NEW.risk_score, 'risk_level', NEW.risk_level) );
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
DO $$ BEGIN RAISE NOTICE 'placeholder 20251107000003'; END $$;
-- PubliMicro User Verification & Background Check System
-- Migration: Add user verification with automatic and manual approval workflows
-- Date: 2025-11-07
